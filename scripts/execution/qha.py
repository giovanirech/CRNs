...
def pre_optimization ( atoms ):
	...
	calc = GULP(keywords='conp opti conj isotropic', options=['maxcyc 2000', 'switch lbfgs gnorm 0.01', 'gmax 0.01', 'gtol 0.01', 'ftol 0.001', 'output cif pre.cif'],  library='brenner', conditions=c)
...
def phonon_calculation(atoms, factor_dir):
	...
	calc = GULP(keywords='phon noden prop', options=['shrink 6 6 6', 'output freq text ' + freq_file ], library='brenner', conditions=c)
	atoms.set_calculator(calc)
...
def fit_volume_vs_temperature(T, V):
    ...
    params = Parameters()
    params.add('a0', min=-10, max=100000)
    params.add('a1', min=-3000, max=3000)
    params.add('a2', min=-3000, max=3000)
    params.add('a3', min=-3000, max=3000)

    minimizer = Minimizer(func2min, params, fcn_args=(T, V))
    kws={'popsize':500, 'mutation':(0.8,1.2), 'recombination':0.8}
