    ...
    if args.F != None:
        ...
        for i in range(len(args.F)):
            ...
            for j in range(len(v_list)):
                ...
                for k in range(total_number_of_kpoints):
                    for f in range(total_number_of_modes):
                        F_thermal += kpoints_weights[k] * Kb_eV_per_K*temp * np.log(1-np.exp(-(h_eV_s*c_cm_per_s*data[j][k][f])/(Kb_eV_per_K*temp)))

                        F_thermal_smooth += kpoints_weights[k]*Kb_eV_per_K*temp*np.log(1-np.exp(-(h_eV_s*c_cm_per_s*data_smooth[j][k][f])/(Kb_eV_per_K*temp)))
